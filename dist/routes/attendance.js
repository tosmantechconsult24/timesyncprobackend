...
router.post('/record', async (req, res) => {
    try {
        const { employeeId, // Can be UUID or employee number
        type, // 'clock_in' or 'clock_out' or 'CLOCK_IN' or 'CLOCK_OUT'
        timestamp, verificationMethod = 'FINGERPRINT', location = 'Kiosk', terminalId, confidence } = req.body;
        ...
        try {
            ...
            if (normalizedType === 'CLOCK_IN') {
                ...
                if (!existingEntry) {
                    ...
                    logger_1.logger.info(`TimeEntry created for clock in: ${employee.employeeId}`);
                }
            }
            else if (normalizedType === 'CLOCK_OUT') {
                // Find the clock in entry for today
                const timeEntry = await prisma_1.prisma.timeEntry.findFirst({
                    where: {
                        employeeId: employee.id,
                        clockIn: { gte: attendanceToday },
                        clockOut: null
                ...
                if (timeEntry) {
                    ...
                    logger_1.logger.info(`TimeEntry updated for clock out: ${employee.employeeId} (${regularHours.toFixed(2)} hours)`);
                }
            }
        }
        catch (timeEntryError) {
            logger_1.logger.error('Error creating/updating TimeEntry:', timeEntryError);
...
router.post('/clock-in', async (req, res) => {
    try {
        ...
        try {
            ...
            logger_1.logger.error('Error creating TimeEntry:', timeEntryError);
        }
        ...
        logger_1.logger.error('Clock in error:', error);
...
router.post('/clock-out', async (req, res) => {
    try {
        ...
        try {
            ...
            logger_1.logger.error('Error updating TimeEntry:', timeEntryError);
        }
        // Emit real-time event
        if (global.io) {
            global.io.to('attendance-updates').emit('attendance:event', {
        ...
        logger_1.logger.error('Clock out error:', error);
...
router.get('/', (0, auth_1.requirePermission)('attendance:read'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { employeeId, departmentId, startDate, endDate, eventType, limit = '100', offset = '0' } = req.query;
    const where = {};
    if (employeeId) {
    ...
    const [records, total] = await Promise.all([
        ...
        prisma_1.prisma.attendanceLog.count({ where }),
    ]);
    res.json({ records, total, limit: parseInt(limit), offset: parseInt(offset) });
}));
// Get today's attendance summary
router.get('/summary/today', (0, auth_1.requirePermission)('attendance:read'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);
    // Get today's clock-ins
    const clockIns = await prisma_1.prisma.attendanceLog.findMany({
        where: {
            eventType: 'CLOCK_IN',
            timestamp: {
                gte: today,
...
router.get('/summary/monthly', (0, auth_1.requirePermission)('attendance:read'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { departmentId, month, year } = req.query;
    const startDate = new Date(parseInt(year) || new Date().getFullYear(), (parseInt(month) || new Date().getMonth()), 1);
...
router.post('/manual', (0, auth_1.requirePermission)('attendance:write'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { employeeId, type, timestamp, notes } = req.body;
    if (!employeeId || !type || !timestamp) {
        throw new errorHandler_1.AppError('employeeId, type, and timestamp are required', 400);
    }
    const employee = await prisma_1.prisma.employee.findFirst({
        where: {
            OR: [
...
router.delete('/:id', (0, auth_1.requirePermission)('attendance:delete'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const record = await prisma_1.prisma.attendanceLog.findUnique({
        where: { id: req.params.id },
    });
    if (!record) {
        throw new errorHandler_1.AppError('Attendance record not found', 404);
    }
    await prisma_1.prisma.attendanceLog.delete({
        where: { id: req.params.id },
    });
    await prisma_1.prisma.auditLog.create({
        data: {
